...
async function verifyPhase4() {
  ...
  console.log('NIST 800-171 Rev 3 - Phases 1-4 Cumulative Verification');
  console.log('='.repeat(80));
  console.log('');
  ...
  try {
    ...
    // Get Phase 4 controls specifically
    const maControls = await prisma.control.findMany({
      where: { family: 'MA' },
      select: { controlId: true, title: true, priority: true },
      orderBy: { controlId: 'asc' }
    });

    const mpControls = await prisma.control.findMany({
      where: { family: 'MP' },
      select: { controlId: true, title: true, priority: true },
      orderBy: { controlId: 'asc' }
    });

    console.log('📋 Phase 4 Controls:');
    console.log('');
    console.log('MA (Maintenance) - 3 controls:');
    maControls.forEach(c => {
      console.log(`   ${c.controlId} - ${c.title} [${c.priority}]`);
    });
    console.log('');
    console.log('MP (Media Protection) - 7 controls:');
    mpControls.forEach(c => {
      console.log(`   ${c.controlId} - ${c.title} [${c.priority}]`);
    });
    console.log('');
    ...
    const maCount = maControls.length;
    const mpCount = mpControls.length;
    ...
    console.log('');
    console.log(`Total: ${totalCount} controls (expected: 59) ${totalCount === 59 ? '✅' : '❌'}`);
    console.log('');
    ...
    if (totalCount === expectedTotal &&
        ...
        maCount === 3 && mpCount === 7) {
      console.log('='.repeat(80));
      console.log('🎉 SUCCESS: Phases 1-4 verification passed!');
      console.log('='.repeat(80));
      console.log('');
      console.log('Summary:');
      ...
      console.log('');
      console.log('🎯 Progress: 59/97 controls (60.8%)');
      console.log('');
      console.log('Remaining: 38 controls across 4 phases');

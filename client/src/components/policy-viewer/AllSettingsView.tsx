...
// Main Component
const AllSettingsView: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['all-settings-to-controls'],
    queryFn: () => policyViewerService.getAllSettingsToControls(),
  ...
  const filteredSettings = useMemo(() => {
    ...
    return data.settings.filter((setting) => {
      ...
      if (filters.searchQuery) {
        ...
        const matchesPolicy = setting.policyName?.toLowerCase().includes(query);
        if (!matchesName && !matchesDescription && !matchesPath && !matchesPolicy) {
          return false;
        }
      }
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', mb: 2 }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {data.summary.total} policy settings mapped to controls across all policies
              </Typography>
            </Box>
          </Box>
